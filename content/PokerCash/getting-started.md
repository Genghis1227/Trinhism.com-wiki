...
5. Click **Add Player**. 
6. Once added, their payment handles automatically format as clickable profile links in your roster table for easy reference. You can edit player details at any time by clicking the **pencil icon** next to their row, or click the **QR code icon** to display an on-screen profile QR code for instant scan-to-pay at the table (supports Venmo and PayPal).
...
2. Under **Chip Value Matrix**, customize the denominations assigned to each color (defaults to micro stakes cents: Green=$0.25, Blue=$0.50).
...
6. **Copy & Text**: Click **Copy Settlement** to copy a text ledger. It generates and appends pre-filled mobile payment URLs (with transaction type, amount, and notes) directly next to each debtor instruction, allowing players to pay in one tap from their chat application.
